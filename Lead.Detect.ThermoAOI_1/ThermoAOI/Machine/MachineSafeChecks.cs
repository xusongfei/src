        ...
        public override string Description
        {
            ...
            switch (Target)
            {
                ...
                case SafeCheckType.Auto:
                    if (!Up.LocateInPos(""Wait""))
                    {
                        Error = ""{Up.Name} 不在等待位置"";
                        return false;
                    }
                    else if (!Down.LocateInPos(""Wait""))
                    {
                        Error = ""{Down.Name} 不在等待位置"";
                        return false;
                    }
                    else if (!DownGT.WaitDi(null, false, 1))
                    {
                        Error = ""{Down.Name} GT不在等待位置"";
                        return false;
                    }
                    return true;
                ...
                case SafeCheckType.Manual:
                    ...
                    else if (Carrier.LocateInPos(""Work""))
                    {
                        if (i < 2)
                        {
                            ...
                            {
                                Error = ""{down.Name} Z 不在等待位"";
                                return false;
                            }
                            if (!DownGt.WaitDi(null, false, 1))
                            {
                                Error = ""{down.Name} GT 不在等待位"";
                                return false;
                            }
                            return true;
                        }
